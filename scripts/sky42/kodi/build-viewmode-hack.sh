...
cp ${WHERE}/kodi-100.42-viewmodes-240813.patch ${FHS}/packages/mediacenter/kodi/patches/ || exit $?
sed -i -E 's|(BUILDER_VERSION="[0-9]{6})"$|\1z"|' ${FHS}/distributions/LibreELEC/version || exit $?

# b42 is my build wrapper script (in this case for Generic-BT2020, RPi4, RPi5 as they are my main devices)
for i in  b 4 5 ; do b42 $i || exit $? ; done
